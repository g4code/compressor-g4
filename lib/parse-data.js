...
ParseData.prototype = {
    ...
    onEachBlock: function(rawBlock)
    {
        evento.trigger(messages.informerLoading, 'Analyzing blocks')
        var oneBlock = new Block(rawBlock, this.options).run();
        this.layoutData = this.layoutData.replace(oneBlock.raw, oneBlock.embedCode);
    },

    run: function()
    {
        evento.trigger(messages.informerLoading, 'Analyzing layout')
