...
Block.prototype = {
    ...
    getEmbedCodeCss: function()
    {
        return this.oneBlock.source == 'inline' ?
            ...
            '<link rel="stylesheet" type="text/css" href="' + this.options.compressedFilePrefix + '/' + this.oneBlock.relativeName + '" />'
    },
    ...
    getEmbedCodeJs: function()
    {
        return this.oneBlock.source == 'inline' ?
            ...
            '<script type="text/javascript" src="' + this.options.compressedFilePrefix + '/' + this.oneBlock.relativeName + '" ' + (this.oneBlock.dataMain === undefined ? '' : 'data-main="' + this.options.compressedFilePrefix + '/' + this.oneBlock.dataMain + '"') + '></script>'
    },
    ...
    moveFilesToTmp: function()
    {
        if (this.oneBlock.type == "js") {
            var filesLength = this.oneBlock.files.length
            for (var i = 0; i < filesLength; i++) {
                this.moveOneFile(this.oneBlock.files[i], i)
            }
        }
        return this
    },
    ...
    parse: function()
    {
        ...
        var blockParts = this.oneBlock.raw.split("\n")
        var blockPartsLength = blockParts.length
        for (var i = 0; i < blockPartsLength; i++) {
            this.parseOneLine(blockParts[i])
        }
        return this;
    },
